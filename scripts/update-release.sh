...
main() {
  ...
  local release tag version src_url src_sum
  ...
  src_url="https://github.com/${UPSTREAM_REPO}/archive/refs/tags/${tag}.tar.gz"

  printf 'checking maestro %s source package\n' "$version"
  src_sum="$(download_sha256 "$src_url")"

  update_pkgbuild "$version" "$src_sum"
  (cd "$ROOT_DIR" && makepkg --printsrcinfo > .SRCINFO)

  printf 'updated maestro to %s\n' "$version"
}
